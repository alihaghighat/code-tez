...
def calculate_pearson(x, y):
    """
    Calculate Pearson correlation using Numba for faster performance.
    """
...
def process_row_multiprocessing(args):
    ...
    # print(f"Processing row: {row_index}")  # Debug: Current row being processed
    ...
    for other_stock, other_stock_data in other_stocks_data.items():
        ...
        # Calculate correlation using Numba-optimized function
        correlation = calculate_pearson(window_1, window_2)
    ...
    # if top_correlated_stocks:
    # # print(f"    Top correlated stocks: {[(stock, corr) for stock, corr, _ in top_correlated_stocks]}")  # Debug: Top stocks
...
class StockCorrelationProcessor:
    ...
    def create_time_based_sliding_window(self, n_steps, stock_name, date_step='1D', n_top=3, delay=1, start_date='2018-01-01'):
        """
        Create a time-based sliding window with multiprocessing and Numba.
        """
